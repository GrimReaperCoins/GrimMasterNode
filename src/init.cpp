...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Grim.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Grimd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeGrimamount=<n> " + _("Keep N Grim anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Grim is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Grim version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    nAnonymizeGrimAmount = GetArg("-anonymizeGrimamount", 0);
    if(nAnonymizeGrimAmount > 999999) nAnonymizeGrimAmount = 999999;
    if(nAnonymizeGrimAmount < 2) nAnonymizeGrimAmount = 2;
    ...
    LogPrintf("Anonymize Grim Amount %d\n", nAnonymizeGrimAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1GRIM+1000 == (.1GRIM+100)*10
       10GRIM+10000 == (1GRIM+1000)*10
    */
