...
namespace {
    class CGrimAddressVisitor : public boost::static_visitor<bool> {
    private:
        CGrimAddress *addr;
    public:
        CGrimAddressVisitor(CGrimAddress *addrIn) : addr(addrIn) { }
...
bool CGrimAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CGrimAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CGrimAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CGrimAddressVisitor(this), dest);
}

bool CGrimAddress::IsValid() const {
...
CTxDestination CGrimAddress::Get() const {
    if (!IsValid())
...
bool CGrimAddress::GetKeyID(CKeyID &keyID) const {
...
bool CGrimAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CGrimSecret::SetKey(const CKey& vchSecret) {
...
CKey CGrimSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CGrimSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CGrimSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CGrimSecret::SetString(const std::string& strSecret) {
