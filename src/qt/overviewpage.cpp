    ...
    filter(0)
{
	...
	ui->listTransactions->setStyleSheet("color: #000000; background-image:url(\":/images/hand\"); background-repeat: no-repeat;  background-position: bottom; border: none; padding-top: 0px; padding-bottom: 0px;");
...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeGrimAmount = BitcoinUnits::formatWithUnit(false, nAnonymizeGrimAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeGrimAmount = strAnonymizeGrimAmount.remove(strAnonymizeGrimAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeGrimAmount + " GRIM / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeGrimAmount*COIN) nMaxToAnonymize = nAnonymizeGrimAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeGrimAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeGrimAmount));
        strAnonymizeGrimAmount = strAnonymizeGrimAmount.remove(strAnonymizeGrimAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeGrimAmount + " GRIM / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
