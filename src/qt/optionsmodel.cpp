...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeGrimAmount"))
        settings.setValue("nAnonymizeGrimAmount", 1000);
    nAnonymizeGrimAmount = settings.value("nAnonymizeGrimAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeGrimAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeGrimAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeGrimAmount:
            return QVariant(nAnonymizeGrimAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeGrimAmount:
            nAnonymizeGrimAmount = value.toInt();
            settings.setValue("nAnonymizeGrimAmount", nAnonymizeGrimAmount);
            emit AnonymizeGrimAmountChanged(nAnonymizeGrimAmount);
            break;
        default:
            break;
        }
